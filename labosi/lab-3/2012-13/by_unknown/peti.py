...
class Atom(Komponenta):
    ...
    def value(self):
        ...
        print ("Greska! Ne postoji vrijednost pridruzena simbolu:" + self.atom)
        sys.exit(-1)
...
class KompozitPlus(Kompozit):
    ...
    def __str__(self):
        return str(self.lijeviIzraz.value() + self.desniIzraz.value())
    ...
    for operator in ["+-", "*/"]:
        ...
        for p in range(len(strInput) - 1, -1, -1):
            if strInput[p] == ')': depth += 1
                ...
                return KompozitFactory(strInput[p], parseExpression(strInput[:p]), parseExpression(strInput[p+1:]))
